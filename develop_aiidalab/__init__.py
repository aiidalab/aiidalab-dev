...
import click
import toml
from aiidalab.utils import load_app_registry_entry, load_app_registry_index
...
def config():
    ...
    cfg = {
        key[len("AIIDALAB_") :].lower(): getattr(aiidalab.config, key)
        for key in dir(aiidalab.config)
        if key.startswith("AIIDALAB_")
    }
        ...
        if paths.home_app_user.is_symlink():
            if paths.home_app_user.resolve() == paths.aiidalab_home_dev.resolve():
                ...
                raise click.ClickException(
                    f"Link {paths.home_app_user} exists and points to unexpected location."
                )
        else:
            raise click.ClickException(
                f"{paths.home_app_user} exists and is not expected symbolic link."
            )
    else:
        click.echo(f"Link {paths.home_app_user} -> {paths.home_app_system}")
        paths.home_app_user.symlink_to(paths.home_app_system)
...
def status(ctx):
    ...
    if dev_mode:
        ...
        if (
            paths.aiidalab_home_dev_aiidalab_link.resolve()
            == paths.aiidalab_dev_package.resolve()
        ):
            msg_ok(
                f"Link {paths.aiidalab_home_dev_aiidalab_link} to {paths.aiidalab_dev_package} is set."
            )
        else:
            msg_issue(
                f"Link {paths.aiidalab_home_dev_aiidalab_link} to {paths.aiidalab_dev_package} is missing."
            )
        ...
        if paths.home_app_user.resolve() == paths.aiidalab_home_dev.resolve():
            ...
            msg_issue(
                f"Link {paths.home_app_user} to {paths.aiidalab_home_dev} is missing."
            )
    elif system_mode:
        ...
        msg_issue(
            "The home app is installed, but is either a local directory or points to an unknown location."
        )
...
def setup(ctx, github_username, use_ssh):
    ...
    aiidalab_url = (
        f"git@github.com:{github_username}/aiidalab.git"
        if use_ssh
        else f"https://github.com/{github_username}/aiidalab.git"
    )
    aiidalab_home_url = (
        f"git@github.com:{github_username}/aiidalab-home.git"
        if use_ssh
        else f"https://github.com/{github_username}/aiidalab-home.git"
    )
    ...
    if paths.aiidalab_home_dev_aiidalab_link.exists():
        ...
        click.echo(
            f"Link {paths.aiidalab_home_dev_aiidalab_link} -> {paths.aiidalab_dev_package}"
        )
        paths.aiidalab_home_dev_aiidalab_link.symlink_to(
            paths.aiidalab_dev_package, target_is_directory=True
        )
    ...
    if paths.home_app_user.exists():
        if paths.home_app_user.is_symlink():
            if paths.home_app_user.resolve() == paths.home_app_system.resolve():
                ...
                paths.home_app_user.symlink_to(
                    paths.aiidalab_home_dev, target_is_directory=True
                )
            elif paths.home_app_user.resolve() != paths.aiidalab_home_dev.resolve():
                raise click.ClickException(
                    f"Link {paths.home_app_user} already exists and points at unknown location."
                )
